...
| [Развёртывание](docs/deploy.md) | Как поднять приложение на своём сервере: Docker, nginx, реплики и что нужно от эксплуатации |
...
из 22 Старших Арканов, атлас из 91 направления, выбор направления,
...
**Превью-сборки для веток работают.** Пуш в любую ветку собирает отдельный
preview-деплой со своим адресом — его видно в `npx vercel ls tutu-tarot`, а на
упавшую сборку приходит письмо. Логи конкретной сборки:
`npx vercel inspect --logs <адрес деплоя>`.

Это единственная автоматическая проверка, которая у нас есть, и она ловит
больше, чем кажется: сборка на Vercel может упасть там, где локальная
проходит. Ровно так и случилось с `output: "standalone"` — локально он
собирался под обеими настройками, а на Vercel ронял их сборочный адаптер
(см. комментарий в `next.config.ts`). Поэтому в ветку стоит пушить до того,
как считать работу законченной.
...
Приложение при этом больше не привязано к Vercel: в коде нет ни одной ссылки
на его переменные, шрифты лежат в репозитории, ссылка шеринга строится из
адреса страницы. В репозитории есть `Dockerfile` и `docker-compose.yml`,
которые поднимают его на своём сервере — несколько реплик за nginx. Подробно,
с объяснением, что именно раньше молча делала платформа, — в
